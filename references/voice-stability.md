...
4. 每集先生成 3 个确定性 seed，必要时最多 5 个。每个候选必须先做 ASR/forced alignment，再做 raw voice-stability 检查。
5. 候选排序固定为：硬门禁全部通过 → 与黄金 prompt/上一块的声学状态最接近 → 所需增益修正最小 → 局部包络最稳 → CPM 最接近。不能只按语速选择。
...
- 2.5 秒声学基线滑窗的 F0 最大步进 ≤ 4 半音、P95 ≤ 2.5 半音；
- 同一滑窗的 spectral-centroid 最大步进 ≤ 4 半音、P95 ≤ 3 半音。
...
<voxcpm-python> videos/skill-beginner-chapter-03-series/generate_all_voxcpm2.py \
  --mode episode-take --episode 2 --target-cpm 330 --candidate-count 3
